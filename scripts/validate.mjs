...
if (marketplace) {
  ...
  if (!marketplace.description) fail('Marketplace description is required.');
  ...
  if (entry?.strict !== true)
    fail('Marketplace leadtime plugin must use strict mode.');
  if (entry?.version && !isStrictSemver(entry.version))
    fail('Marketplace plugin version must be strict semver.');
}
...
const claudeValidation = spawnSync(
  'claude',
  ['plugin', 'validate', projectRoot, '--strict'],
  {
    cwd: repoRoot,
    encoding: 'utf8',
  },
...
if (claudeValidation.error?.code === 'ENOENT') {
  ...
  fail(
    [
      ...
      claudeValidation.stdout.trim(),
      claudeValidation.stderr.trim(),
    ]
      .filter(Boolean)
      .join('\n'),
  );
